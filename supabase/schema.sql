-- ==============================================================================
-- KASIR TOKO SEMBAKO - SUPABASE MASTER DATABASE SCHEMA
-- ==============================================================================

-- ------------------------------------------------------------------------------
-- 1. TABEL PROFILES & AUTH
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 4. SEQUENCE & KODE BARANG OTOMATIS (products)
-- ------------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS public.products (
    ...
    minimum_stock NUMERIC(14,3) DEFAULT 0 CHECK (minimum_stock >= 0),
    status BOOLEAN NOT NULL DEFAULT true,
...
-- ------------------------------------------------------------------------------
-- 6. TABEL RIWAYAT HARGA (product_price_history)
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 7. TABEL BARANG BELUM TERDAFTAR (unregistered_prices)
-- ------------------------------------------------------------------------------
...
CREATE POLICY "Owner can view all profiles" ON public.profiles FOR SELECT TO authenticated USING (public.is_owner());
...
CREATE POLICY "Categories can be managed by owner" ON public.categories FOR ALL TO authenticated USING (public.is_owner()) WITH CHECK (public.is_owner());
...
CREATE POLICY "Units can be managed by owner" ON public.units FOR ALL TO authenticated USING (public.is_owner()) WITH CHECK (public.is_owner());
...
CREATE POLICY "Products can be managed by owner" ON public.products FOR ALL TO authenticated USING (public.is_owner()) WITH CHECK (public.is_owner());

-- Price History Policies
DROP POLICY IF EXISTS "Price history is viewable by authenticated users" ON public.product_price_history;
CREATE POLICY "Price history is viewable by authenticated users" ON public.product_price_history FOR SELECT TO authenticated USING (true);

-- Unregistered Prices Policies
