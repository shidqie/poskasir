    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-5 max-w-2xl mx-auto w-full">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500">Catat perubahan stok barang fisik secara manual</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        {success && (
          <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 flex items-center gap-3">
            ...
            <button onClick={() => setSuccess(null)} className="ml-auto text-emerald-600 hover:text-emerald-800 text-xs font-bold cursor-pointer">Tutup</button>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl border border-slate-200/90 p-5 space-y-3 shadow-xs">
          ...
          {searchTerm.trim().length > 1 && !selectedProduct && (
            <div className="border border-slate-200 rounded-xl overflow-hidden max-h-48 overflow-y-auto">
              ...
              {products.map((p) => (
                ...
                >
                  <span className="font-bold text-slate-800 text-left truncate">{p.name}</span>
                  <span className="text-xs text-slate-500 shrink-0 ml-2 font-semibold">Stok: {p.stock}</span>
                </button>
              ))}
            </div>
          )}
          ...
          {/* Barang terpilih */}
          {selectedProduct && (
            <div className="bg-red-50/70 border border-red-200 rounded-xl px-4 py-3 flex items-center justify-between">
              <div>
                <p className="text-sm font-bold text-red-900">{selectedProduct.name}</p>
                <p className="text-xs text-red-700 font-medium">Stok saat ini: {selectedProduct.stock}</p>
              </div>
              <button onClick={() => { setSelectedProduct(null); setSearchTerm(''); }}
                className="text-xs text-red-600 hover:text-red-800 font-bold cursor-pointer">Ganti</button>
            </div>
          )}
        </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200/90 p-5 space-y-4 shadow-xs">
              ...
              <div>
                ...
                <input
                  ...
                  min="0"
                  step="0.001"
                  className="w-full px-4 py-3 border border-slate-200 rounded-xl text-lg font-black text-right outline-none focus:border-red-500"
                />
              </div>
              <div>
              ...
              {adjustMutation.isError && (
                <p className="text-xs text-rose-600 font-semibold text-center">{adjustMutation.error?.message}</p>
              )}
            </div>
          </>
        )}
